...
import numpy as np
import pickle
# from .dataset_utils import transform_to_target_pos, transform_to_target_coord, random_scale, random_rotation, is_near, trajectories_filtering, random_flip, to_trajectory, target_in_bound
...
class ETHUCYSTARDataset(moai_social_bertDataset):
    ...
    def get_data_index(self, data_dict, setname, ifshuffle=True):
        ...
        # To make full use of the data, extend the tail so trailing samples are not dropped.
        if setname == 'train':
    ...
    def get_seq_from_index_balance(self, frameped_dict, pedtraject_dict, data_index, setname):
        ...
        for i in range(data_index.shape[1]):
            ...
            # assert False
            present_pedi = framestart_pedi | frameend_pedi # intersection
