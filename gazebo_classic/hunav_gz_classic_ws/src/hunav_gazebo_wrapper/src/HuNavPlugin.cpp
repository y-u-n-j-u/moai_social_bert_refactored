...
void HuNavPlugin::Load(gazebo::physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
  ...
  hnav_->useRobot =
      !(hnav_->robotName.empty() || hnav_->robotName == "none" || hnav_->robotName == "None" ||
        hnav_->robotName == "NONE");
  if (!hnav_->useRobot)
  {
    RCLCPP_INFO(hnav_->rosnode->get_logger(),
                "HuNav plugin running without a robot model. A far-away dummy robot agent will be used.");
    hnav_->robotAgent.id = -1;
    hnav_->robotAgent.type = hunav_msgs::msg::Agent::ROBOT;
    hnav_->robotAgent.behavior.type = hunav_msgs::msg::AgentBehavior::BEH_REGULAR;
    hnav_->robotAgent.behavior.state = hunav_msgs::msg::AgentBehavior::BEH_NO_ACTIVE;
    hnav_->robotAgent.name = "none";
    hnav_->robotAgent.group_id = -1;
    hnav_->robotAgent.radius = 0.0;
    hnav_->robotAgent.position.position.x = 1000000.0;
    hnav_->robotAgent.position.position.y = 1000000.0;
    hnav_->robotAgent.yaw = 0.0;
    hnav_->robotAgent.linear_vel = 0.0;
...
bool HuNavPluginPrivate::InitializeRobot()
{
  ...
  RCLCPP_INFO_THROTTLE(rosnode->get_logger(), *rosnode->get_clock(), 2000, "Initializing robot...");
  robotModel = world->ModelByName(robotName);
  if (!robotModel)
  {
    RCLCPP_ERROR_THROTTLE(rosnode->get_logger(), *rosnode->get_clock(), 2000,
                          "Robot model %s not found (yet)!!!!", robotName.c_str());
    return false;
  }
  else
  {
