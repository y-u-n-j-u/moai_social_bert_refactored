...
namespace hunav
{
  ...
  {
    RCLCPP_INFO(get_logger(), "Waiting for hunav_loader hunav_parameters service...");
    
    // Wait for service to be available with timeout
    if (!get_parameters_client_->wait_for_service(std::chrono::seconds(60))) {
      RCLCPP_ERROR(get_logger(), "Service /hunav_parameters not available after waiting");
      return false;
    }
    ...
    if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), future, std::chrono::seconds(5)) ==
      ...
      RCLCPP_ERROR(get_logger(), "Failed to call service /hunav_parameters");
      return false;
    }
  }
