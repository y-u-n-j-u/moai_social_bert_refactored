...
from .indicator import Spatial6DIndicator, IntegerIndicator, SpatialIndicator
from .util import *
import json

class ETHUCYTPPDataset(moai_social_bertDataset):
    def __init__(self, split, args):
        ...
        for data in tqdm.tqdm(self.dataset, total=len(self.dataset)):
            ...
            first_idx = int(first_history_index)
            tgt_obs_traj = x_t.numpy().copy()
            tgt_pred_traj = y_t.numpy().copy()
            nbr_data = neighbors_data[("PEDESTRIAN", "PEDESTRIAN")]
            nbr_lower_upper = neighbors_lower_upper[("PEDESTRIAN", "PEDESTRIAN")]
            ...
            for nbr_obs, nbr_obs_st, nbr_lower in zip(nbr_data, neighbors_data_st[('PEDESTRIAN', 'PEDESTRIAN')], nbr_lower_upper):

                nbr_obs = nbr_obs[:, :self.args.input_dim].clone()
