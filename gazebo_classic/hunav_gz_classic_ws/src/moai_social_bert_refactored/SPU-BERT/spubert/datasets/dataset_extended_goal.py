...
import tqdm
from .viz import *
from .indicator import SpatialIndicator, IntegerIndicator

class moai_social_bertDataset(Dataset):
    def __init__(self, split, args):
        self.trajs = []
        self.path = os.path.join(args.dataset_path, args.dataset_name)
        self.seq_len = args.obs_len + args.pred_len
        self.seq_input_len = (args.obs_len+1) * (args.num_nbr + 1) + args.pred_len + 1
        self.all_trajs = []
        self.all_scenes = []
        self.scene_images = {}
        self.envs = {}
    ...
    def __getitem__(self, item):
        trajs = self.all_trajs[item].copy()
        scale = 1.0/self.scales[self.all_scenes[item]]
        if self.args.scene:
            env = self.envs[self.all_scenes[item]].copy()
        ################################################
        ...
        if self.args.mode == "pretrain":
            ...
            tgt_traj = trajs[0, :self.seq_len].copy()
            ...
            traj_lbl = trajs[0, :self.seq_len].tolist()

            nbr_obs_trajs = trajs[1:, :self.args.obs_len, :].copy()
            is_near_lbl = is_near_lbl[1:]
            near_lbl = []
            for nbr_idx, nbr_traj in enumerate(nbr_obs_trajs):
                nbr_traj_lbl = nbr_traj.copy()
            ...
            trajs[1:, self.args.obs_len:, :] = np.nan
            tgt_pred_traj = trajs[0, self.args.obs_len:].copy()
            ...
            if ext_goal_world is not None:
                # trans = center = -obs_end_world, rot = theta (heading angle)
                ct = np.cos(rot)
                st = np.sin(rot)
                r = np.array([[ct, st], [-st, ct]])
                p_shifted = ext_goal_world + trans
                goal_lbl = p_shifted.dot(r.T).astype(np.float32)
            else:
            ...
            mgp_spatial_ids = [self.s_ind.sot_id] + tgt_obs_traj[:self.args.obs_len].tolist() + [self.s_ind.pad_id] * (
                        self.args.pred_len) + [self.s_ind.msk_id]
            mgp_segment_ids = [1] + [self.i_ind.pad_id if nan else 1 for nan in nan_idx[:self.args.obs_len]] + [
                self.i_ind.pad_id] * (self.args.pred_len) + [1]
            mgp_temporal_ids = [self.i_ind.pad_id] + [self.i_ind.pad_id if nan else x for x, nan in
                                                      zip(range(1, self.args.obs_len + 1),
                                                          nan_idx[:self.args.obs_len])] + [self.i_ind.pad_id] * (
                                           self.args.pred_len) + [self.seq_len]
            mgp_attn_mask = [self.i_ind.true_val] + [self.i_ind.false_val if nan else self.i_ind.true_val for nan in
                                                     nan_idx[:self.args.obs_len]] + [self.i_ind.false_val] * (
                                        self.args.pred_len) + [self.i_ind.true_val]
            ...
            tgp_spatial_ids = [self.s_ind.sot_id] + tgt_obs_traj[:self.args.obs_len].tolist() + [self.s_ind.msk_id] * self.args.pred_len + [goal_lbl.tolist()]
            tgp_segment_ids = [1] + [self.i_ind.pad_id if nan else 1 for nan in nan_idx[:self.args.obs_len]] + [1] * self.args.pred_len + [1]
            tgp_temporal_ids = [self.i_ind.pad_id] + [self.i_ind.pad_id if nan else x for x, nan in zip(range(1, self.args.obs_len+1), nan_idx[:self.args.obs_len])] + [x for x in np.arange(self.args.obs_len+1, self.seq_len+1)] + [self.seq_len + 1]
            tgp_attn_mask = [self.i_ind.true_val] + [self.i_ind.false_val if nan else self.i_ind.true_val for nan in nan_idx[:self.args.obs_len]] + [self.i_ind.true_val] * self.args.pred_len + [self.i_ind.true_val]
