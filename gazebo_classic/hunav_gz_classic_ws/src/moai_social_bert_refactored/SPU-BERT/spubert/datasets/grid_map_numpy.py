...
# Keep the full grid for each angle, derive angle indices from point directions,
# and identify the nearest cells for each angle bucket.
...
class RectangularGridMap:
    ...
    def copy(self):
        ...
        copied.none_val = self.none_val
        copied.none_idx = self.none_idx
        copied.none_pos = self.none_pos
        copied.grid_map = self.grid_map.copy()
        return copied
