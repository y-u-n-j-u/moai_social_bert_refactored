...
pilot_name="${HUNAV_PILOT_NAME:-intersection_safe_teacher_v2}"
output_dir="$recordings_dir/$pilot_name"
container_output_dir="/home/hunav_gz_classic_ws/moai_recordings/$pilot_name"
scenario="${HUNAV_PILOT_SCENARIO:-agents_training_intersection_safe_teacher_low.yaml}"
seeds_text="${HUNAV_PILOT_SEEDS:-101 102 103 104 105 106 107 108 109 110}"
run_timeout="${HUNAV_PILOT_RUN_TIMEOUT:-150}"
...
map_yaml="$workspace_dir/src/hunav_gazebo_wrapper/maps/training_intersection.yaml"

mkdir -p "$output_dir"
read -r -a seeds <<< "$seeds_text"
# Ten deterministic encounter profiles vary arrival time and crossing offset.
# HUNAV_AUTO_GOAL_SEED alone does not randomize HuNav's manual SFM scenario.
start_x_profiles=(5.2 5.4 5.6 5.8 6.0 6.2 6.4 6.6 6.8 7.0)
robot_y_profiles=(-0.2 0.0 0.2 -0.2 0.0 0.2 -0.2 0.0 0.2 0.0)
run_index=0
...
for seed in "${seeds[@]}"; do
    profile_index=$((run_index % ${#start_x_profiles[@]}))
    start_x="${start_x_profiles[$profile_index]}"
    robot_y="${robot_y_profiles[$profile_index]}"
    goal_waypoints="-6.0,${robot_y};${start_x},${robot_y}"
    ...
    for ((attempt = 1; attempt <= max_attempts; attempt++)); do
        ...
        echo "[safe-teacher] seed=$seed profile=$profile_index attempt=$attempt start=($start_x,$robot_y) scenario=$scenario"
        (
            HUNAV_DOCKER_TTY=false \
            HUNAV_DOCKER_USE_GPU=false \
            HUNAV_SCENARIO="$scenario" \
            HUNAV_NAVIGATION=True \
            HUNAV_PMB2_NAV_PARAMS_FILE=/home/hunav_gz_classic_ws/install/hunav_gazebo_wrapper/share/hunav_gazebo_wrapper/launch/pmb2_params/pmb2_nav_safe_teacher.yaml \
            ...
            HUNAV_AUTO_GOAL_MIN_EPISODE_DURATION=8.0 \
            HUNAV_AUTO_GOAL_TIMEOUT=55.0 \
            HUNAV_AUTO_GOAL_NO_PROGRESS_TIMEOUT=20.0 \
        ...
        while kill -0 "$launcher_pid" >/dev/null 2>&1; do
            ...
            if (( SECONDS >= deadline )); then
                echo "[safe-teacher] seed=$seed attempt=$attempt timed out" >&2
                break
            fi
            sleep 2
        done
    ...
    if [ "$completed" != true ] || [ ! -s "$raw_path" ]; then
        echo "[safe-teacher] seed=$seed produced no completed recording" >&2
        continue
    fi
